...
export default ( state = initialState, action, service ) => {
    switch (action.type) {
		...
		case "UI_ASK_DEUCE":
            return updateState(state, s => askDeuce(s));
		case "UI_END_GAME":
            return updateState(state, s => endGame(s));
		case "UI_MOVE_START":
            return updateState(state, s => moveStart(s, action));
		case "UI_MOVE_END":
        ...
        case "DEUCE":
            return updateState(state, s => acceptRemoteDeuce(s));
        case "LOGIN_OK":
            return updateState(state, s => loginOk(s, action));
        default:
            return state;
    }
}
...
const playersRemove = (state, action) => {
    ...
    state.waitingPlayers.forEach(i => {
        if (state.player !== i && !action.payload.players.includes(i)) { 
            p.push(i);
        }
    });
    state.waitingPlayers = p;
...
const loginOk = (state, action) => { state.player = action.payload.from; }
...
const acceptRemoteDeuce = state => {
    state.myMove = true; 
    state.confirmDeuce = false; 
    state.endGame = true; 
    state.message = 'Deuce';
    amendLastMove(state, {payload:{text:'deuce'}});
}

const sendEndGameMessage = (state, msg, suffix) => {
    state.myMove = true; 
    state.message = msg; 
    state.endGame = true;
    amendLastMove(state, {payload:{text:suffix}});
}
